...
class Utils {
  static Future<void> manipulateSplashData(BuildContext context) async {
    ...
    if (strUser != null) {
      ...
      Nav.navigateTo( SelectLang(), navigatorType: NavigatorType.push);
    }
  }

  static getSliders(BuildContext context) async {
    List<String> images = [];
    List<SliderModel> sliders = await CustomerRepository(context).getSliders();
    sliders.forEach((element) {
      images.add(element.image!);
    });
    context.read<SliderCubit>().onUpdateLSliders(images);
  }
  ...
  static Future<bool> manipulateLoginData(
      BuildContext context, dynamic data, String token,
      {bool fromSendCode = false}) async {
    if (data != null) {
      bool status = data['activeCode'];
      if (status) {
        ...
        CustomToast.showSimpleToast(
            msg: tr(context,
                fromSendCode ? 'accountActivated' : 'successfulLogin'));
        Nav.navigateTo( Home(), navigatorType: NavigatorType.pushAndPopUntil);
      } else {
        CustomToast.showSimpleToast(
            msg: tr(context, 'thisAccountIsNotActivated'));
        Nav.navigateTo( ActiveAccount(userId: data["id"]),
  ...
  static void shareApp(url) {
    ...
    SharePlus.instance.share(ShareParams(text: url)).whenComplete(() {
      EasyLoading.dismiss();
    });
  }
